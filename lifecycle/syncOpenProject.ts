...
const ghIssues: {
  ...
  "https://api.github.com/repos/stoked/for-web/issues?per_page=100&sort=created",
).then((r) => r.json());
  ...
  if (issue.ghIssue) {
    ...
    if (ghIssue) {
      ...
      if (!ghIssue.title.startsWith(PREFIX)) {
        ...
        await fetch(
          `https://api.github.com/repos/stoked/for-web/issues/${ghIssue.number}`,
          {
            method: "PATCH",
            headers: {
    ...
    const result: { html_url: string } = await fetch(
      `https://api.github.com/repos/stoked/for-web/issues`,
      {
        method: "POST",
        headers: {
          Accept: "application/vnd.github+json",
